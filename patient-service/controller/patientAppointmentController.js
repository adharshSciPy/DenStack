...
const createAppointment = async (req, res) => {
  ...
  try {
    ...
    // 2️⃣ Combine date and time into a Date object for comparison
    ...
    // Check for past date/time (disallow appointments in the past)
    ...
    // 3️⃣ Verify that the patient exists in the same clinic
    ...
    // 4️⃣ Verify receptionist belongs to this clinic
    ...
    const isReceptionistInClinic = clinic.staffs.receptionists.some(
    ...
    const availabilities = availRes.data?.doctors?.[0]?.availability || [];
    ...
    const isAvailable = availabilities.some((slot) => {
      if (slot.clinicId.toString() !== clinicId.toString() || slot.dayOfWeek !== appointmentDay)
        return false;
    ...
    // 7️⃣ Prevent double booking at same date and time
