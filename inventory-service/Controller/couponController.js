...
export const getAllCoupons = async (req, res) => {
  try {
    ...
    // Attach derived status to each coupon for convenience
...
export const updateCoupon = async (req, res) => {
  try {
    ...
    if (usageLimit !== undefined && usageLimit < coupon.usedBy.length) {
      return res.status(400).json({
        success: false,
        message: `Usage limit cannot be less than current usage count (${coupon.usedBy.length})`,
      });
    }
...
// ─── Admin: Generate a fresh unique code ────────────────────────────────────
...
export const getCouponUsers = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      usageCount: coupon.usedBy.length,
      data: coupon.usedBy,
    });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
...
export const validateCoupon = async (req, res) => {
  try {
    const { code, email, name, orderAmount, orderId, userId } = req.body;
...
// ─── User: Apply / Redeem Coupon (records usage) ────────────────────────────
...
export const applyCoupon = async (req, res) => {
  try {
    ...
    if (!code || !email || !name) {
      return res.status(400).json({
        success: false,
        message: "Coupon code, email, and name are required",
      });
    }
...
export const getCouponStats = async (req, res) => {
  try {
    ...
    const exhausted = allCoupons.filter(
      (c) => c.usedBy.length >= c.usageLimit
    ).length;
    ...
    return res.status(200).json({
      success: true,
      data: { total, active, expired, exhausted, totalUses },
    });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
